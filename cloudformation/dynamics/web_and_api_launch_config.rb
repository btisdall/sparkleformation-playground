...
SparkleFormation.dynamic(:web_and_api_launch_config) do |_name, _config = {}|
...
EOF
  ...
  resources("#{_name}_launch_config".to_sym) do
    ...
    properties do
      image_id _config[:image_id] || _cf_map(:region_map, 'AWS::Region', :ami)
      instance_type _config[:instance_type]
      associate_public_ip_address _config(:public_ips)
